...
import {
  CreateCollectionBlockchains,
  CreateCollectionRequest,
} from "@rarible/sdk/build/types/nft/deploy/domain";
...
import {
  resultToState,
  useRequestResult,
} from "../../components/hooks/use-request-result";
  ...
  return (
    <Page header="Deploy Collection">
        ...
        >
          <Stack spacing={2}>
            <FormSelect
              ...
              label="Blockchain"
            >
              <MenuItem value={Blockchain.ETHEREUM}>
                {Blockchain.ETHEREUM}
              </MenuItem>
              <MenuItem value={Blockchain.POLYGON}>
                {Blockchain.POLYGON}
              </MenuItem>
      ...
      <CommentedBlock
        sx={{ my: 2 }}
        comment={
          result.type === "complete" ? <CollectionResultComment /> : null
        }
      >
        <RequestResult
          result={result}
          completeRender={(data) => (
            <>
              <Box sx={{ my: 2 }}>
                ...
                <div>
                  <InlineCode>{data?.address}</InlineCode>{" "}
                  <CopyToClipboard value={data?.address} />
                </div>
              </Box>
              <Box sx={{ my: 2 }}>
